...
pub mod actors {
    ...
    use crate::{subspacevole::{RAAACode, calc_consistency_check, LinearCode}, FVec, FMatrix, zkp::{R1CS, quicksilver::{ZKP, self}, R1CSWithMetadata}, vecccom::{commit_seeds, commit_seed_commitments, proof_for_revealed_seed, reconstruct_commitment}, smallvole::{ProverSmallVOLEOutputs, self, VOLE}, challenges::{challenge_from_seed, calc_quicksilver_challenge, calc_other_challenges}, NUM_VOLES, PF};
...
impl<T: PF> Prover<T> {
    ...
    pub fn from_witness_and_circuit_unpadded(mut witness: FVec<T>, mut circuit: R1CSWithMetadata<T>) -> Self {
...
impl<T: PF> Verifier<T> {
    ...
    pub fn verify(&self, cnp: &CommitAndProof<T>) -> Result<PublicUOpenings<T>, Error> {
        ...
        for i in 0..self.num_voles {
            ...
            hasher.update(&rec);
            let vole_outs = sv.verifier_outputs(&proof.seed_openings.seed_opens[i], challenges.delta_choices[i] == 0, self.vole_length);
...
pub mod test_helpers {
    ...
    use crate::{FVec, zkp::R1CSWithMetadata, Fr};

    use super::actors::{Prover, Verifier, PublicUOpenings};

    pub fn e2e_test(witness: FVec<Fr>, circuit: R1CSWithMetadata<Fr>) -> Result<PublicUOpenings<Fr>, Error>{
...
mod test {
    use anyhow::anyhow;
    use ff::{PrimeField, Field};
    use crate::{subspacevole::RAAACode, zkp::{self, R1CSWithMetadata}, actors::{actors::{Prover, Verifier, CommitAndProof}, test_helpers::e2e_test}, Fr, FVec, circom::witness};
    use super::*;
    ...
    fn prover_verifier_full_integration_tiny_circuit() {
        ...
        let correct_witness = FVec::<Fr>(vec![5, 2, 28, 280].iter().map(|x|Fr::from_u128(*x)).collect());
    ...
    fn public_values() {
        let circuit = zkp::test::TEST_R1CS_WITH_METADA.clone();
        let witness = FVec::<Fr>(vec![5, 2, 28, 280].iter().map(|x|Fr::from_u128(*x)).collect());
