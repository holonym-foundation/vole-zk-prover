...
pub fn expand_seed_to_Fr_vec(seed: [u8; 32], log_length: u32) -> Vec<Fr> {
...
mod test {
    ...
    fn test_seed_expansion_len() {
        let seed = [0u8; 32];
        assert_eq!(super::expand_seed_to_Fr_vec(seed.clone(), 0).len(), 1);
        assert_eq!(super::expand_seed_to_Fr_vec(seed.clone(), 1).len(), 2);
        assert_eq!(super::expand_seed_to_Fr_vec(seed.clone(), 2).len(), 4);
        assert_eq!(super::expand_seed_to_Fr_vec(seed.clone(), 10).len(), 1024);
    }
}